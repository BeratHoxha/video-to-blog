...
const PLANS = [
  ...
  },
  {
    ...
    price: "$12",
    period: "/mo",
    features: ["Unlimited words", "Unlimited AI rewrites", "PDF & DOCX export", "Priority processing"],
  },
  {
    key: "premium",
    label: "Premium",
    price: "$29",
    period: "/mo",
    features: ["Everything in Basic", "PowerPoint export", "Custom tone settings", "Early access to features"],
  },
];

export function ProfilePage({ user, csrfToken, initialTab = "account", onUserUpdate }: ProfilePageProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-2xl mx-auto px-8 py-10">
        ...
        {activeTab === "account" ? (
          <AccountTab user={user} csrfToken={csrfToken} onUserUpdate={onUserUpdate} />
        ) : (
          <PlanTab user={user} csrfToken={csrfToken} onUserUpdate={onUserUpdate} />
        )}
      </div>
    </div>
  );
...
const inputClass =
  "w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-colors";

const inputDisabledClass =
  "w-full bg-gray-800/40 border border-gray-700/40 rounded-lg px-4 py-2.5 text-sm text-gray-500 cursor-not-allowed";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-start gap-3">
        <button
          onClick={handleSave}
          disabled={saving}
          className="inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-500 hover:bg-emerald-400 active:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-lg shadow-md shadow-emerald-900/40 transition-colors cursor-pointer"
        >
            ...
            >
              ...
              <div className="w-44 shrink-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <h3 className={`text-sm font-bold ${isPremium ? "text-amber-400" : isBasic ? "text-sky-400" : "text-white"}`}>
                    {plan.label}
                  </h3>
                  {isPremium && !isCurrent && (
